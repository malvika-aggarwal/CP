angular.module('careersupportCtrl', [])
    .controller('careerSupportController', ['$scope', '$http', 'Notification', '$sce', '$location', function ($scope, $http, Notification, $sce, $location) {
        ...
        $scope.saveSection = function () {
            ...
            $http.post('/saveCarrerSupport', data).then((data) => {
                Notification.success('Section successfully created');
                $('#sectionCreateModal').modal('hide');
                $scope.clearValues();
                $scope.careerSupportInit();
            }).catch((error) => {
                Notification.error(error.data);
            })
        };

        $scope.careerSupportInit = function () {
            $scope.showMediaChange = false;
            $http.get('/getSectionsForCareerSupport').then((sections) => {
        ...
        $scope.updateSection = function () {
            ...
            $http.put(`/updateSectionCareerSupport/${$scope.sectionData._id}`, $scope.sectionData).then(() => {
        ...
        $scope.careersupportUserInit = function () {
            $http.get(`/getCareerSupportListing?lang=${$scope.currentLanguage}`).then((data) => {
